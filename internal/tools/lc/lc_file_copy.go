...
// LcFileCopyParams represents the parameters for copying a file
type LcFileCopyParams struct {
...
// LcFileCopyResult represents the result of a copy operation
type LcFileCopyResult struct {
	AppName     string `json:"app_name"`
	SourcePath  string `json:"source_path"`
	DestPath    string `json:"dest_path"`
...
// LcFileCopy copies a file within an app directory
func LcFileCopy(params LcFileCopyParams) (LcFileCopyResult, error) {
	if params.AppName == "" {
		return LcFileCopyResult{}, errors.New("app_name is required")
	}
	if params.SourcePath == "" {
		return LcFileCopyResult{}, errors.New("source_path is required")
	}
	if params.DestPath == "" {
		return LcFileCopyResult{}, errors.New("dest_path is required")
	}

	// Validate paths don't contain directory traversal
	if strings.Contains(params.SourcePath, "..") || strings.Contains(params.DestPath, "..") {
		return LcFileCopyResult{}, errors.New("directory traversal is not allowed")
	}

	// Get and validate the apps directory
	appsDir, err := config.EnsureAppsDirectory()
	if err != nil {
		return LcFileCopyResult{}, fmt.Errorf("failed to ensure apps directory: %w", err)
	}
	...
	if !config.IsWithinDirectory(cleanSourcePath, appPath) || !config.IsWithinDirectory(cleanDestPath, appPath) {
		return LcFileCopyResult{}, errors.New("paths must be within the app directory")
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return LcFileCopyResult{}, fmt.Errorf("source file not found: %s", params.SourcePath)
		}
		return LcFileCopyResult{}, fmt.Errorf("error accessing source file: %w", err)
	}

	// Don't allow copying directories
	if sourceInfo.IsDir() {
		return LcFileCopyResult{}, errors.New("copying directories is not supported")
	}

	// Check file size limit
	if sourceInfo.Size() > constants.MaxFileSize {
		return LcFileCopyResult{}, fmt.Errorf("file exceeds maximum size of %s", constants.MaxFileSizeInWords)
	}

	// Prevent copying file to itself
	if cleanSourcePath == cleanDestPath {
		return LcFileCopyResult{}, errors.New("source and destination are the same")
	}

	// Check if destination exists
	if _, err := os.Stat(cleanDestPath); err == nil && !params.Overwrite {
		return LcFileCopyResult{}, fmt.Errorf("destination already exists: %s (use overwrite option to replace)", params.DestPath)
	}

	// Create destination directory if needed
	destDir := filepath.Dir(cleanDestPath)
	if err := os.MkdirAll(destDir, 0755); err != nil {
		return LcFileCopyResult{}, fmt.Errorf("failed to create destination directory: %w", err)
	}

	// Open source file
	sourceFile, err := os.Open(cleanSourcePath)
	if err != nil {
		return LcFileCopyResult{}, fmt.Errorf("failed to open source file: %w", err)
	}
	...
	if err != nil {
		return LcFileCopyResult{}, fmt.Errorf("failed to create destination file: %w", err)
	}
	...
	if err != nil {
		...
		return LcFileCopyResult{}, fmt.Errorf("failed to copy file: %w", err)
	}
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			if strings.HasPrefix(args[i], "--") {
				return fmt.Errorf("unknown option: %s\nRun 'layered-code tool lc_file_copy --help' for usage", args[i])
			}
		}
	}
...
func printLcFileCopyHelp() {
	fmt.Println("Usage: layered-code tool lc_file_copy [options]")
	fmt.Println()
	...
	fmt.Println("  layered-code tool lc_file_copy --app-name myapp --source config.json --dest config.backup.json")
	fmt.Println()
	fmt.Println("  # Copy and overwrite existing file")
	fmt.Println("  layered-code tool lc_file_copy --app-name myapp --from template.html --to index.html --overwrite")
}

// MCP
func LcFileCopyMcp(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var params LcFileCopyParams
